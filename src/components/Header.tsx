...
export const Header: React.FC<HeaderProps> = ({ theme, matches, totalPairs }) => {
  ...
  return (
    <header className="w-full px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between bg-white/95 sticky top-0 z-30 shadow-md border-b border-white/50">
      ...
      <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl text-center text-purple-800 flex-1 px-2 sm:px-3 drop-shadow-sm truncate">
        {theme.name}
      </h1>

      <div className="flex items-center gap-0.5 sm:gap-1 bg-white/90 px-2 sm:px-3 py-1.5 sm:py-2 rounded-full shadow-md border border-yellow-200 shrink-0">
